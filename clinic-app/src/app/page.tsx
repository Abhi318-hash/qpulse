...
import {
  ...
  subscribeToUserActiveTokens,
  addPatientToken,
  cancelUserToken,
  getUserMedicalHistory,
  saveFcmToken,
  ensurePatientProfile,
  subscribeToPatientsAheadCount,
  ...
  return (
    <div
      style={{
        textAlign: "center",
        marginTop: "1rem",
        background: count === 0 ? "#d1fae5" : "#fffbeb",
        padding: "0.6rem",
        borderRadius: 8,
        color: count === 0 ? "#059669" : "#f59e0b",
        fontSize: "0.85rem",
        fontWeight: 700,
      }}
    >
...
function useIOSInstallPrompt() {
  ...
  useEffect(() => {
    ...
    const dismissed =
      localStorage.getItem("qpulse_ios_prompt_dismissed") === "true";
    ...
    >
      <div style={{ flex: 1 }}>
        ...
        >
          ...
          below, then select <b>"Add to Home Screen"</b>.
        </p>
      </div>
      <button
        onClick={dismiss}
    ...
    <Suspense
      fallback={
        <div style={{ display: "grid", placeItems: "center", height: "100vh" }}>
          <Loader2 className="animate-spin" style={{ color: "#007BFF" }} />
        </div>
      }
    >
      <HomeContent />
...
function HomeContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      ...
      if (user && user.phoneNumber) {
        ...
        unsubscribeTokens = subscribeToUserActiveTokens(
          user.phoneNumber,
          (tokens) => {
            setMyTokens(tokens);
  ...
  useEffect(() => {
    const unsubscribeClinics = subscribeToActiveClinics(
      (data) => {
        data.forEach((newClinic) => {
          const oldClinic = clinicsRef.current.find(
            (c) => c.id === newClinic.id,
          );
          if (
            oldClinic &&
            oldClinic.currently_serving_token !==
              newClinic.currently_serving_token
          ) {
            const matchingToken = myTokensRef.current.find(
              (t) => t.clinic_id === newClinic.id,
            );
            if (matchingToken && newClinic.currently_serving_token !== "--") {
              ...
              if (!isNaN(servingNum) && !isNaN(userNextTokenNum)) {
                if (servingNum === userNextTokenNum) {
                  triggerNotification(
                    "🚨 Turn Alert!",
                    `Your token #${userNextTokenNum} is now being called!`,
                  );
                } else if (userNextTokenNum - servingNum === 2) {
                  triggerNotification(
                    "⚠️ Be Ready",
                    `You are next in line at token #${servingNum}!`,
                  );
                }
              }
            }
  ...
  const handleBookClick = useCallback(
    (clinic: any) => {
      ...
      if (existing) {
        alert(
          `You already have active Token #${existing.token_number} at this clinic!`,
        );
        return;
      }
  ...
  const submitBooking = async (e: React.FormEvent) => {
    ...
    if (useInsurance && (!insuranceProvider || !insuranceImage)) {
      alert(
        "Please provide your insurance provider and upload a photo of your ID card.",
      );
      return;
    }
    if (
    ...
    try {
      ...
      if (useInsurance && insuranceImage) {
        ...
        const fileRef = ref(
          storage,
          `insurance/${currentUser.uid}/${Date.now()}.${fileExt}`,
        );
  ...
  const favoriteClinics = useMemo(
    () => filteredClinics.filter((c) => favorites.includes(c.id)),
    [filteredClinics, favorites],
  );
      ...
      >
            ...
            >
              ...
              {showNavDropdown && (
                ...
                >
                  ...
                  >
                    <LanguageSelector isMobile={false} isDark={isDark} />
                  </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                Real-Time Queue Network — Live Now
              </span>
            </div>
            <h1
              style={{
            ...
            >
              Book your clinic token online and track the live queue in real
              time — no calls, no waiting room stress.
            </p>

            {/* ── MOVIE-BOOKING STYLE DISCOVERY ── */}
        ...
        >
          ...
          {currentUser && myHistory.length > 0 && (
            <section style={{ marginBottom: "3rem" }}>
              ...
              <div
                ...
                className="hide-scrollbar"
              >
                {Array.from(new Set(myHistory.map((h) => h.clinic_id))).map(
                  (cid) => {
                    ...
                    return (
                      <div
                        key={cid}
                        onClick={() => router.push(`#clinic-${cid}`)}
                        style={{
                          cursor: "pointer",
                          minWidth: 260,
                          background: "#fff",
            ...
            <section style={{ marginBottom: "3rem" }}>
              ...
              >
                {myTokens.map((token) => {
                  ...
                  const positionsAhead = myTokensRef.current.filter(
                    (t) =>
                      t.clinic_id === token.clinic_id &&
                      Number(t.token_number) < Number(token.token_number),
                  ).length;
                  return (
                    <div
                      key={token.id}
                      style={{
                        ...
                        padding: "1.5rem",
                        boxShadow:
                          "0 2px 8px rgba(0,0,0,0.06), 0 8px 24px rgba(0,123,255,0.08)",
                    ...
                    >
                      ...
                      {token.queued_at && mc && (
                        <TokenAheadCounter
                          ...
                          queuedAtMillis={token.queued_at.toMillis()}
                        />
                      )}
                      ...
                      {token.consultation_type === "VIDEO" &&
                        ((mc &&
                          Number(token.token_number) -
                            Number(mc.currently_serving_token) <=
                            0) ||
                          token.status === "SERVING") && (
                          <Link
                            href={`/consult/${token.id}`}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              justifyContent: "center",
                          ...
                          >
                            <span style={{ fontSize: "1.2rem" }}>📹</span> Join
                            Video Call Now
                          </Link>
                        )}
                      ...
                      >
                        <button
                          onClick={async () => {
                            ...
                            try {
                              ...
                              alert(
                                "Access granted! Doctor can now securely view your past medical history.",
                              );
                            } catch (err) {
                        ...
                        <button
                          onClick={async () => {
                            if (!currentUser) return;
                            try {
                              await revokeClinicAccess(
                                currentUser.uid,
                                token.clinic_id,
                              );
                              alert(
                                "Access revoked! Doctor can no longer see your history.",
                              );
                            } catch (err) {
                      ...
                      <button
                        onClick={() =>
                          handleCancelToken(token.clinic_id, token.id)
                        }
                        style={{
                          width: "100%",
                          marginTop: "0.5rem",
              ...
              <div
                style={{
                  ...
                  height: 1,
                  background:
                    "linear-gradient(90deg,transparent,#dee2e8,transparent)",
                }}
              />
            </section>
          )}
          ...
          <section style={{ marginBottom: "4rem" }}>
            ...
            >
              ...
              >
                Available Clinics in {selectedCity || "Your Area"}
              </h2>
            </div>
              ...
              {(() => {
                ...
                let filtered = clinics.filter((c) => {
                  ...
                  const matchCategory =
                    ...
                    c.specialization === selectedCategory;
                  ...
                  <>
                    {isFallback && (
                      ...
                      >
                        ...
                        >
                          No {selectedCategory} found in {selectedCity}.
                        </strong>
                          ...
                          >
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  borderRadius: "50%",
                                  background:
                                    "linear-gradient(135deg,#e8f4fd,#cce4f8)",
                                  display: "flex",
                                  alignItems: "center",
                                  justifyContent: "center",
                                }}
                            ...
                            >
                              <div>
                                ...
                                >
                                  #{clinic.currently_serving_token || "--"}
                                </strong>
                              </div>
                            ...
                            <button
                              ...
                              disabled={
                                !clinic.is_open || clinic.booking_closed
                              }
        ...
        {bookingClinic && (
          <div
            ...
            onClick={(e) =>
              e.target === e.currentTarget && setBookingClinic(null)
            }
          >
            <div
              style={{
            ...
            >
                ...
                >
                  ...
                  >
                    {bookingClinic.doctor_image_url ? (
                      <Image
                        src={bookingClinic.doctor_image_url}
                        alt=""
                        fill
                  ...
                  <div>
                    ...
                    >
                      {bookingClinic.doctor_name} ·{" "}
                      {bookingClinic.specialization || "General Physician"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                >
                  Fill in your details to generate a live token:
                </p>
                ...
                >
                  <button
                    ...
                    className={`btn ${consultationType === "IN_PERSON" ? "btn-primary" : "btn-outline"}`}
                  ...
                  <button
                    ...
                    className={`btn ${consultationType === "VIDEO" ? "btn-primary" : "btn-outline"}`}
                    style={{ flex: 1, padding: "0.75rem", fontSize: "0.85rem" }}
                  >
                    📹 Video Consult
                  </button>
                ...
                >
                  You&apos;ll be assigned a live token. Pay ₹
                  {bookingClinic.fees || "500"} at the clinic desk.
                  <div
                    style={{
                      fontSize: "0.72rem",
                      color: "#007BFF",
                ...
                <div
                  style={{
                    ...
                    border: "1px solid #eef0f3",
                    borderRadius: 10,
                    background: useInsurance ? "#f0f9ff" : "transparent",
                  }}
                >
                  <label
                    style={{
                  ...
                  {useInsurance && (
                    ...
                    >
                      ...
                      <input
                        ...
                        accept="image/*"
                        onChange={(e) =>
                          setInsuranceImage(e.target.files?.[0] || null)
                        }
                        required
                        disabled={isBooking}
                ...
                {!bookingClinic.is_open ? (
                  ...
                  >
                    ...
                    >
                      This clinic is not accepting new tokens right now. Please
                      try again later.
                    </p>
                  </div>
                ) : (
              ...
              >
                {myHistory.length === 0 ? (
                  ...
                  myHistory.map((record) => {
                    const dateObj = record.created_at?.toDate
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          >
                            {record.patient_name} ({record.age}yrs) ·{" "}
                            {record.disease}
                          </p>
                        </div>
                        <div
                        ...
                        >
                          <p
                            style={{ margin: "0 0 0.2rem 0", fontWeight: 600 }}
                          >
...
const ClinicCard = React.memo(function ClinicCard({
  clinic,
  isFavorite,
  onFavoriteToggle,
  onBookClick,
  ...
  const patientUrl = useMemo(
    () =>
      typeof window !== "undefined"
        ? `${window.location.origin}/?addFavorite=${clinic.id}`
        : "",
    [clinic.id],
  );
  ...
  const initials = (clinic.doctor_name || "DR")
    .split(" ")
    .map((w: string) => w[0])
    .join("")
    .toUpperCase()
  ...
  if (clinic.booking_end_time) {
    ...
    if (
      now.getHours() > hr ||
      (now.getHours() === hr && now.getMinutes() >= min)
    ) {
    ...
    <div
      ...
      onMouseEnter={(e) => {
        ...
        (e.currentTarget as HTMLDivElement).style.boxShadow =
          "0 4px 12px rgba(0,0,0,0.08), 0 16px 40px rgba(0,123,255,0.1)";
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLDivElement).style.transform = "";
        (e.currentTarget as HTMLDivElement).style.boxShadow =
    ...
    >
      ...
      >
        <button
          ...
          style={{
            background: isFavorite ? "rgba(220,53,69,0.1)" : "#f8fafc",
            border: `1px solid ${isFavorite ? "rgba(220,53,69,0.3)" : "#eef0f3"}`,
            borderRadius: "50%",
            cursor: "pointer",
            padding: "0.4rem",
            display: "flex",
        ...
        >
          <Star
            ...
            color={isFavorite ? "#dc3545" : "#5a6a7e"}
          />
        </button>
        <button
          onClick={(e) => {
          ...
          style={{
            background: showQR ? "rgba(0,123,255,0.1)" : "#f8fafc",
            border: `1px solid ${showQR ? "rgba(0,123,255,0.3)" : "#eef0f3"}`,
            borderRadius: "50%",
            cursor: "pointer",
            padding: "0.4rem",
            display: "flex",
      ...
      <div style={{ padding: "1.5rem" }}>
        ...
        >
          ...
          >
            {clinic.doctor_image_url ? (
              <img
                src={clinic.doctor_image_url}
                alt={clinic.doctor_name}
                style={{ width: "100%", height: "100%", objectFit: "cover" }}
              />
            ) : (
              <span
                style={{ fontWeight: 700, fontSize: "1rem", color: "#007BFF" }}
              >
                {initials}
              </span>
            )}
        ...
        <div
          style={{
            ...
            borderRadius: 12,
            border: `1px solid ${isOpen ? "rgba(0,123,255,0.12)" : "#eef0f3"}`,
            marginBottom: "1.25rem",
          }}
        >
          <div style={{ textAlign: "center" }}>
          ...
          <div
            style={{
              width: 1,
              background: isOpen ? "rgba(0,123,255,0.15)" : "#eef0f3",
            }}
          />
          <div style={{ textAlign: "center" }}>
            <span
        ...
        {showDetails && (
          ...
          >
            <a
              href={`https://maps.google.com/?q=${encodeURIComponent(clinic.location)}`}
              target="_blank"
              rel="noopener noreferrer"
              style={{
                display: "flex",
            ...
            >
              <MapPin size={14} color="#007BFF" style={{ flexShrink: 0 }} />{" "}
              {clinic.location}
            </a>
            <div
              style={{
            ...
            >
              <Clock size={14} color="#007BFF" style={{ flexShrink: 0 }} />{" "}
              {hours}
            </div>
            <a
              href={`tel:${phone}`}
            ...
            >
              <Phone size={14} color="#007BFF" style={{ flexShrink: 0 }} />{" "}
              {phone}
            </a>
            {clinic.booking_end_time && (
              ...
              >
                <Clock size={14} color="#dc3545" style={{ flexShrink: 0 }} />{" "}
                Booking Cutoff: {clinic.booking_end_time}
              </div>
            )}
            <div
              style={{
            ...
            >
              <IndianRupee
                ...
                style={{ flexShrink: 0 }}
              />{" "}
              ₹{fees} per consultation · Pay at desk
            </div>
          </div>
        )}
        ...
        <button
          ...
          disabled={!isOpen || isBookingClosed}
          className="btn btn-primary"
          style={{
            width: "100%",
            padding: "0.85rem",
            ...
            fontWeight: 700,
            marginTop: "1rem",
            opacity: !isOpen || isBookingClosed ? 0.5 : 1,
            justifyContent: "center",
          }}
        >
          {isOpen ? (
            isBookingClosed ? (
              ...
              <>
                <CalendarPlus size={17} /> Book Token Now
              </>
            )
          ) : (
            "Currently Closed"
        ...
        {showQR && (
          ...
          >
            ...
            >
              Scan to add this clinic to favourites on another device
            </p>
            <div
              style={{
                background: "#f8fafc",
...
function RecentVisitUploadPrompt({
  visit,
  currentUser,
  onUploadSuccess,
}: {
  ...
  if (dismissed || visit.prescription_uploaded || visit.status !== "COMPLETED")
    return null;

  // Only show if completed in the last 48 hours
  const completedDate = visit.completed_at?.toDate
    ? visit.completed_at.toDate()
    : visit.created_at?.toDate
      ? visit.created_at.toDate()
      : new Date();
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      const { uploadFileToStorage, addPatientPrescriptionDocument } =
        await import("@/lib/patientActions");
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: "0.78rem", color: "#5a6a7e" }}>
            Upload your prescription for your visit on{" "}
            {completedDate.toLocaleDateString()} to save it to your secure
            health history.
          </p>
        </div>
      </div>
